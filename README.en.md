...
- [What AI Manor can do](#what-ai-manor-can-do)
...
## What AI Manor can do

| You ask | AI Manor does |
|---|---|
| "Sort out what needs doing for next month's move" | Breaks it into tasks with due dates, gathered into a project |
...
Each member of staff can only write to their own tables. See [`docs/staff/`](docs/staff/) for what each can be
asked; `manor talk <name>` talks to one directly, without the butler.
...
| **Phone access** Tailscale | `tailscale serve` plus a passcode. No public URL is created | [`docs/tailscale.md`](docs/tailscale.md) |
