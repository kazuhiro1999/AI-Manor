...
*朝いちばんに開く画面。判断待ち・今日の期限・次に来るものが並ぶ。
以下の画面例はすべて `manor init --demo` の**架空の家庭**のものです。*
...
| | |
|---|---|
| ![台所](docs/screenshots/kitchen.png) | ![タスク](docs/screenshots/tasks.png) |
| **台所** — 在庫は期限順、買い物リストは売り場別、食事は記録として残る | **タスク** — 判断が要ることは承認待ちとして積まれ、その場で承認・修正・却下できる |
| ![担当](docs/screenshots/agents.png) | ![構成](docs/diagrams/png/manor-architecture.png) |
| **担当** — 誰が何を預かっているか。姿（VRM）を入れた担当は顔が出る | **構成** — 執事と部下、手元のデータベース、Web アプリの関係 |

`docs/diagrams/*.html` を開くと、検索・フォーカス・テーマ切替のできる対話版になります
（一覧は [`docs/diagrams/`](docs/diagrams/)）。
